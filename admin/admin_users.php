...
//-- fin mod : Attach mod ---------------------------------------------------------
	...
	if ( ( $mode == 'save' && isset( $HTTP_POST_VARS['submit'] ) ) || isset( $HTTP_POST_VARS['avatargallery'] ) || isset( $HTTP_POST_VARS['submitavatar'] ) || isset( $HTTP_POST_VARS['cancelavatar'] ) )
	{
		...
		$karma = ( !empty($HTTP_POST_VARS['karma']) ) ? trim(strip_tags( $HTTP_POST_VARS['karma'] ) ) : '';
...
//-- fin mod : birthday --------------------------------------------------------
		...
		validate_optional_fields($icq, $aim, $msn, $yim, $website, $location, $occupation, $interests, $karma, $signature);
...
$user_adr_ban = ( !empty($HTTP_POST_VARS['user_adr_ban']) ) ? intval( $HTTP_POST_VARS['user_adr_ban'] ) : 0;
		...
		if( isset( $HTTP_POST_VARS['avatargallery'] ) || isset( $HTTP_POST_VARS['submitavatar'] ) || isset( $HTTP_POST_VARS['cancelavatar'] ) )
		{
			...
			$karma = htmlspecialchars(stripslashes($karma));
...
//-- modify
			...
			$sql = "UPDATE " . USERS_TABLE . "
				SET " . $username_sql . $passwd_sql . "user_email = '" . str_replace("\'", "''", $email) . "', user_icq = '" . str_replace("\'", "''", $icq) . "', user_website = '" . str_replace("\'", "''", $website) . "', user_occ = '" . str_replace("\'", "''", $occupation) . "', user_from = '" . str_replace("\'", "''", $location) . "', user_interests = '" . str_replace("\'", "''", $interests) . "', karma = '" . str_replace("\'", "''", $karma) . "', user_sig = '" . str_replace("\'", "''", $signature) . "', user_viewemail = $viewemail, user_aim = '" . str_replace("\'", "''", $aim) . "', user_yim = '" . str_replace("\'", "''", $yim) . "', user_msnm = '" . str_replace("\'", "''", $msn) . "', user_attachsig = $attachsig, user_qp_settings = '" . $user_qp_settings . "', user_sig_bbcode_uid = '$signature_bbcode_uid', user_allowsmile = $allowsmilies, user_allowhtml = $allowhtml, user_allowavatar = $user_allowavatar, user_allowbbcode = $allowbbcode, user_allow_viewonline = $allowviewonline, user_notify = $notifyreply, user_allow_pm = $user_allowpm, user_notify_pm = $notifypm, user_popup_pm = $popuppm, user_lang = '" . str_replace("\'", "''", $user_lang) . "', user_style = $user_style, user_timezone = $user_timezone, user_dateformat = '" . str_replace("\'", "''", $user_dateformat) . "', user_active = $user_status, user_rank = $user_rank
 , user_adr_ban = $user_adr_ban" . $avatar_sql . "
				WHERE user_id = $user_id";
//-- mod : flags ---------------------------------------------------------------
//-- add
...
// End Quick Administrator User Options and Information MOD
	...
	else if( !isset( $HTTP_POST_VARS['submit'] ) && $mode != 'save' && !isset( $HTTP_POST_VARS['avatargallery'] ) && !isset( $HTTP_POST_VARS['submitavatar'] ) && !isset( $HTTP_POST_VARS['cancelavatar'] ) )
	{
		...
		$karma = htmlspecialchars($this_userdata['karma']);
//-- mod : flags ---------------------------------------------------------------
//-- add
...
$s_hidden_fields .= '<input type="hidden" name="karma" value="' . str_replace("\"", "&quot;", $karma) . '" />';
//-- mod : flags ---------------------------------------------------------------
//-- add
