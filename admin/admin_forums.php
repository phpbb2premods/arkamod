	...
	switch($mode)
	{
		...
		case 'editforum':
			...
			if ($mode == 'editforum')
			{
				...
				$forumlinkcount = $row['forum_link_count'];
				$forumlinkdate = create_date($board_config['default_dateformat'], $row['forum_link_date'], $board_config['board_timezone']);
...
//-- fin mod : quick post es ---------------------------------------------------
				...
				'L_FORUM_LINK_EXPLAIN' => $lang['Forum_link_explain'],
...
//-- modify
			$sql = "INSERT INTO " . FORUMS_TABLE . " (forum_id, forum_name, cat_id, attached_forum_id, forum_desc, forum_order, forum_status, forum_link, forum_link_date, forum_qpes, forum_icon, prune_enable" . $field_sql . ")
				VALUES ('" . $next_id . "', '" . str_replace("\'", "''", $HTTP_POST_VARS['forumname']) . "', " . intval($HTTP_POST_VARS[POST_CAT_URL]) .  ", " . intval($HTTP_POST_VARS['attached_forum_id']) . ", '" . str_replace("\'", "''", $HTTP_POST_VARS['forumdesc']) . "', $next_order, " . intval($HTTP_POST_VARS['forumstatus']) . ", '" . str_replace("\'", "''", $HTTP_POST_VARS['forumlink']) . "', " . time() . ", " . intval($HTTP_POST_VARS['forum_qpes']) . ", '" . str_replace("\'", "''", $HTTP_POST_VARS['forumicon']) . "', " . intval($HTTP_POST_VARS['prune_enable']) . $value_sql . ")";
//-- fin mod : quick post es ---------------------------------------------------
   			// End Added by Attached Forums MOD
			if( !$result = $db->sql_query($sql) )
			{
	...
	for($i = 0; $i < $total_categories; $i++)
	{
		...
		for($j = 0; $j < $total_forums; $j++)
		{
			...
			if ($forum_rows[$j]['cat_id'] == $cat_id)
			{
				...
				if ($do_template)
				{
				$forum_link_date = create_date($board_config['default_dateformat'], $forum_rows[$j]['forum_link_date'], $board_config['board_timezone']);
				$forum_link = ( !empty($forum_rows[$j]['forum_link']) ) ? append_sid($phpbb_root_path . "forumlink.$phpEx?" . POST_FORUM_URL . "=$forum_id") : append_sid($phpbb_root_path . "viewforum.$phpEx?" . POST_FORUM_URL . "=$forum_id");
				$forum_target = ( !empty($forum_rows[$j]['forum_link']) ) ? 'target="_blank"' : '';
					...
					$template->assign_block_vars("catrow.forumrow",	array(
						...
						'FORUM_LINK' => sprintf($lang['Forum_link_count'], $forum_rows[$j]['forum_link_count'], $forum_link_date),
				...
				if ( !empty($forum_rows[$j]['forum_link']) )
				{
				...
				else
				{
					...
					for($k = 0; $k < $total_forums; $k++)
					{
						...
						{
                     $subforum_date = create_date($board_config['default_dateformat'], $subforum_rows[$k]['forum_link_date'], $board_config['board_timezone']);
                     $subforum_link = ( empty($subforum_rows[$k]['forum_link']) ) ? append_sid($phpbb_root_path . "viewforum.$phpEx?" . POST_FORUM_URL . "=$subforum_id") : append_sid($phpbb_root_path . "forumlink.$phpEx?" . POST_FORUM_URL . "=$subforum_id");
                     $subforum_target =( empty($subforum_rows[$k]['forum_link']) ) ? '' : 'target="_blank"';
							$template->assign_block_vars("catrow.forumrow",	array(
                        ...
                        'FORUM_LINK' => sprintf($lang['Forum_link_count'], $subforum_rows[$k]['forum_link_count'], $subforum_date),
                     ...
                     if( empty($subforum_rows[$k]['forum_link']) )
                     {
                        $template->assign_block_vars('catrow.forumrow.row',array());
                     }
                     else
