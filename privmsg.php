...
//-- fin mod : Attach mod ----------------------------------------------
	...
	else if ( $preview || $refresh || $error )
	{
		...
		//
		$to_username = (isset($HTTP_POST_VARS['username']) ) ? trim(htmlspecialchars(stripslashes($HTTP_POST_VARS['username']))) : '';		$privmsg_subject = ( isset($HTTP_POST_VARS['subject']) ) ? trim(htmlspecialchars(stripslashes($HTTP_POST_VARS['subject']))) : '';
	...
	else 
	{
		...
		else if ( $mode == 'reply' || $mode == 'quote' )
		{
			...
			$privmsg_subject = preg_replace($orig_word, $replacement_word, $privmsg_subject);$to_username = $privmsg['username'];
			$to_userid = $privmsg['user_id'];

			if ( $mode == 'quote' )
			{
				$privmsg_message = $privmsg['privmsgs_text'];				$privmsg_message = preg_replace($orig_word, $replacement_word, $privmsg_message);
