...
class birthday_class
{
	...
	function generate_list($blockname='', $onset=false)
	{
		...
		$counts = array(
			'today' => array('displayed' => 0, 'full_list' => !$max_in_list || ($see == 'today')),
			'lookahead' => array('displayed' => 0, 'full_list' => !$max_in_list || ($see == 'lookahead')),
		);
		...
		if ( !empty($this->config['wishes']) )
		{
			...
			$sql = 'SELECT user_id' . ($this->count_user_fields ? ', ' . implode(', ', $this->user_fields) : '') . '
					FROM ' . USERS_TABLE . '
					WHERE user_birthday LIKE \'' . sprintf('%02d-%02d-', intval($this->now['mon']), intval($this->now['mday'])) . '%\'
