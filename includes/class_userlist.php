...
* @version $Id: class_userlist.php,v 1.1 02/09/2006 18:03 reddog Exp $
...
class today_userlist
{
	...
	function today_userlist()
	{
		...
		$this->ext = substr(strrchr(__FILE__, '.'), 1);
		...
		// ensure rank color system compliancy
	...
	function display()
	{
		...
		while ( $row = $db->sql_fetchrow($result) )
		{
			...
			if ( !$user_style_done && ($display == 'standard') )
			{
				if ( $viewable = (!$hidden || $is_admin) )
				{
					...
					$user_style = array(
						...
						'user_color' => ( !empty($rcs) || is_object($rcs) ) ? intval($row['user_color']) : 0,
						'user_group_id' => ( !empty($rcs) || is_object($rcs) ) ? intval($row['user_group_id']) : 0,
					);
					$style_color = $this->get_user_style($user_style);
					$l_username = sprintf(( $is_admin && $hidden ? '<i>%s</i>' : '%s' ), $row['username']);
					$u_profile = '<a href="' . append_sid('profile.' . $this->ext . '?mode=viewprofile&amp;' . POST_USERS_URL . '=' . intval($row['user_id'])) . '" title="' . $lang['Read_profile'] . '"' . $style_color . '>' . $l_username . '</a>';
				}
			...
			switch ( $display )
			{
				...
				case 'next':
					$l_username = '...';
					$u_profile = '<a href="' . append_sid('index.' . $this->ext . '?see=alltoday') . '" title="' . $lang['View_full_list'] . '">' . $l_username . '</a>';
					$s_today_userlist .= ( $s_today_userlist != '' ) ? ', ' . $u_profile : $u_profile;
					break;
			}
