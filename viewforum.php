		...
		if ($db->sql_numrows ($result2) != 0)
		{
			...
			foreach ($forum_data as $row2 => $value)
			{
				if ($is_auth_ary[$value['forum_id']]['auth_view'])
				{
					...
					$attach_forum_link = ( empty($value['forum_link']) ) ? append_sid('viewforum.php?f=' . $value['forum_id']) : append_sid('forumlink.php?f=' . $value['forum_id']);
...
//-- fin mod : rank color system -----------------------------------------------
            ...
            $forum_target = ( empty($value['forum_link']) ) ? '' : 'target="_blank"';
            $forum_link_date = create_date($board_config['default_dateformat'], $value['forum_link_date'], $board_config['board_timezone']);
				 $template->assign_block_vars ('switch_attached_list.switch_attached_present',
										 ...
										 (
											...
											'FORUM_ICON_IMG' => ($value['forum_icon']) ? '<img src="' . $phpbb_root_path . $value['forum_icon'] . '" alt="'.$value['forum_name'].'" title="'.$value['forum_name'].'" />&nbsp;' : '',   // Forum Icon Mod
