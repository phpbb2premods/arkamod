...
//-- fin mod : rank color system -----------------------------------------------
	...
	case 'ip':
		...
		$ip_this_post = decode_ip($post_row['poster_ip']);		$ip_this_post = ( $rdns_ip_num == $ip_this_post ) ? htmlspecialchars(gethostbyaddr($ip_this_post)) : $ip_this_post;
		...
		if ( $row = $db->sql_fetchrow($result) )
		{
			...
			do
			{
				...
				$ip = decode_ip($row['poster_ip']);				$ip = ( $rdns_ip_num == $row['poster_ip'] || $rdns_ip_num == 'all') ? htmlspecialchars(gethostbyaddr($ip)) : $ip;
...
//-- delete
/*-MOD
		$sql = "SELECT t.*, u.username, u.user_id, p.post_username, p.post_time
