...
if ($arcade_config['championnat_use'] == 1)
{
	...
	if ($liste_userid!='')
	{
		$sql = "SELECT user_id,username,user_level, user_color, user_group_id FROM " . USERS_TABLE . " WHERE user_id IN ($liste_userid)";
...
//-- add
		...
		$tabusers[$row['user_id']] = '<a href="' . append_sid('profile.' . $phpEx . '?mode=viewprofile&amp;' . POST_USERS_URL . '=' . $row['user_id']) . '"' . $style_color . '>' . $row['username'] . '</a>';
//-- fin mod : rank color system -----------------------------------------------
